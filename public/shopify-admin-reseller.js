...
(function() {
    ...
    async function loadProductResellers(productId) {
        try {
            console.log('📡 Loading product resellers for product:', productId);
            const response = await fetch(`${API_BASE_URL}/api/products/${productId}/resellers`);
            if (!response.ok) {
                if (response.status === 404) {
                    console.log('ℹ️ No resellers found for this product (404)');
                    return [];
                }
            ...
            console.log('✅ Loaded product resellers:', resellerIds);
            return resellerIds;
        } catch (error) {
    ...
    async function saveProductResellers(productId, resellerIds) {
        try {
            console.log('💾 Saving resellers for product:', productId, resellerIds);
            const response = await fetch(`${API_BASE_URL}/api/products/${productId}/resellers`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
            console.log('✅ Successfully saved resellers');
            return true;
        } catch (error) {
        ...
        container.innerHTML = resellers.map(reseller => {
            ...
            return `
                ...
                " onclick="toggleResellerSelection(${reseller.id})" 
                   ...
                   onmouseout="this.style.borderColor='${isSelected ? '#007bff' : '#e1e3e5'}'">
                    <input type="checkbox" 
    ...
    async function saveResellers() {
        ...
        try {
            ...
            showMessage(`Successfully updated resellers for this product!`, 'success');
        } catch (error) {
    ...
    async function initialize() {
        ...
        console.log('✅ Data loaded:', { resellers: resellers.length, selected: selectedResellers.length });
