// Shopify Reseller Extension - Content Script
// This automatically adds reseller selection to product pages

(function () {
  "use strict";

  console.log("🏪 Shopify Reseller Extension Loading...");

  // Configuration
  const RESELLER_SECTION_ID = "extension-reseller-selection";
    ...
    section.innerHTML = `
      ...
      <div style="margin-bottom: 20px;">
        ...
        <div style="
          ...
          margin-bottom: 15px;
        ">
          <strong>🚀 Auto-Loaded:</strong> This extension automatically adds reseller selection to all product pages.
        </div>
      </div>
      ...
      <div style="margin-bottom: 20px;">
        ...
        <select id="extension-reseller-select" style="
          width: 100%;
          padding: 12px;
      ...
      ">
        <div id="extension-selected-reseller" style="
          font-size: 13px;
          color: #6c757d;
          flex: 1;
        ...
        <div style="display: flex; gap: 10px;">
          <button id="extension-clear-reseller" style="
            background: #dc3545;
            color: white;
            border: none;
            padding: 8px 16px;
          ...
          <button id="extension-save-reseller" style="
            background: #007bff;
            color: white;
            border: none;
            padding: 10px 20px;
  ...
  function updateSelectedDisplay() {
    const select = document.getElementById("extension-reseller-select");
    const display = document.getElementById("extension-selected-reseller");
  ...
  async function saveReseller() {
    ...
    const saveBtn = document.getElementById("extension-save-reseller");
    ...
    try {
      ...
      console.log("📝 Saving reseller via extension:", selectedReseller);
  ...
  function init() {
    ...
    console.log(
      "✅ Extension: On product editing page, creating reseller UI..."
    );
    ...
    const checkInterval = setInterval(() => {
      ...
      if (productId) {
        const saved = localStorage.getItem(`extension_reseller_${productId}`);
        if (saved) {
          const select = document.getElementById("extension-reseller-select");
          select.value = saved;
      ...
      // Add event listeners
      document.getElementById("extension-reseller-select").onchange =
        updateSelectedDisplay;

      document.getElementById("extension-clear-reseller").onclick = () => {
        const select = document.getElementById("extension-reseller-select");
        select.value = "";
        updateSelectedDisplay();
      };

      document.getElementById("extension-save-reseller").onclick = saveReseller;

      clearInterval(checkInterval);
      console.log("✅ Extension reseller section created successfully!");
    }, 500);
  ...
  console.log("🏪 Shopify Reseller Extension Loaded");
})();
